...
def train_lr_transform(crop_size, upscale_factor,sigma =2.5):
    return Compose([
        ToPILImage(),
        # Apply Gaussian blur with the chosen sigma (radius)
        lambda x: x.filter(ImageFilter.GaussianBlur(radius=sigma)),
...
class TestDatasetFromFolder(Dataset):
    ...
    def __getitem__(self, index):
        ...
        hr_scale = Resize((self.upscale_factor * h, self.upscale_factor * w), interpolation=Image.LANCZOS)
        hr_restore_img = hr_scale(lr_image)
